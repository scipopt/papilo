...
class VeriPb : public CertificateInterface<REAL>
{
 public:
   ...
   print_header()
   {
      ...
      proof_out << COMMENT << "Log files generated by PaPILO " << PAPILO_VERSION_MAJOR << "." << PAPILO_VERSION_MINOR << "." << PAPILO_VERSION_PATCH;
#ifdef PAPILO_GITHASH_AVAILABLE
      proof_out << COMMENT << " [GitHash: " << PAPILO_GITHASH << " ]";
#endif
      proof_out << COMMENT << "\n";
               ...
               const Vec<String>& names, const Vec<int>& var_mapping )
   {
      ...
      proof_out << " >=  "
                << abs( offset ) - num.round_to_int( val ) * scale_factor[row]
                << ";\n";
      rhs_row_mapping[row] = next_constraint_id;
   }
