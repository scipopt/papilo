 ...
 public:
   ...
   {  
      ...
      for( int i = 0; i < cliquelen; ++i )
      {
        ...
        for( int ind = changed_clique_lbs.begin(); ind != changed_clique_lbs.end(); std::advance(ind,1) )
        {
        ...
        for( int ind = changed_clique_ubs.begin(); ind != changed_clique_ubs.end(); std::advance(ind,1) )
        {
...
ProbingView<REAL>::analyzeImplications()
{
   ...
   for( int ind = binary_inds.begin(); ind != binary_inds.end(); ++ind )
   {
      ...
      {
         substitutions.emplace_back(
            ProbingSubstitution<REAL>( binary_inds[ind], -1.0, lb_no_implications[ind].second, 1.0 ) );
      }
   }
   return false;
}
