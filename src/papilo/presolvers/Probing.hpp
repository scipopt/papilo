                        ...
                        const Timer& timer, int& reason_of_infeasibility )
{
   ...
   for( int i = 0; i < static_cast<int>(change_to_equation_comb.size()); ++i )
   {
      ...
      && !num.isGE(consMatrix.getLeftHandSides()[change_to_equation_comb[i]], mincoeff) )
         reductions.changeRowLHS( change_to_equation_comb[i],  mincoeff );
      else if( !num.isLE(consMatrix.getRightHandSides()[change_to_equation_comb[i]], maxcoeff) )
   ...
   for( int i = 0; i < static_cast<int>(change_to_equation.size()); ++i )
   {
      ...
      && !num.isGE(consMatrix.getLeftHandSides()[change_to_equation[i]], mincoeff) )
         reductions.changeRowLHS( change_to_equation[i],  mincoeff );
      else if( !num.isLE(consMatrix.getRightHandSides()[change_to_equation[i]], maxcoeff) )
