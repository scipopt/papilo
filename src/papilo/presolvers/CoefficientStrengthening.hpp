       ...
       [&]( const tbb::blocked_range<int>& r ) {
          for( int j = r.begin(); j < r.end(); ++j )
          {
             ...
             for( std::pair<REAL, int>& intCoef : integerCoefficients )
             {
                ...
                {
                   assert(
                       !domains.flags[intCoef.second].test( ColFlag::kLbInf ) );
             ...
             if( scale == -1 )
             {
                for( const std::pair<REAL, int>& intCoef : integerCoefficients )
                   reduction.changeMatrixEntry( i, intCoef.second,
                                                -intCoef.first );
                assert( rowFlag.test( RowFlag::kRhsInf ) );
                assert( !rowFlag.test( RowFlag::kLhsInf ) );

                if( lhs_values[i] != -rhs )
                   reduction.changeRowLHS( i, -rhs );
             }
             else
             {
                assert( scale == 1 );
