...
if test ! -e $PROJECT_DIR/$BINNAME && ! type $PROJECT_DIR/$BINNAME >/dev/null 2>&1; then
...
if test ! -e $SCIPPATH/$OUTPUTDIR; then
  mkdir $SCIPPATH/$OUTPUTDIR
fi

# create settings directory if non-existent
if test ! -d $PROJECT_DIR/settings/; then
...
for SETNAME in "${SETTINGSLIST[@]}"; do
  SETTINGS="$PROJECT_DIR/settings/${SETNAME}.${SETEXTENSION}"
  if test $SETNAME != "default" && test ! -e $SETTINGS; then
...
if [ -f instancedata/testsets/$TSTNAME.ttest ]; then
  FULLTSTNAME="instancedata/testsets/$TSTNAME.ttest"
  TIMEFACTOR=$TIMELIMIT
else
  if [ -f instancedata/testsets/$TSTNAME.test ]; then
    FULLTSTNAME="instancedata/testsets/$TSTNAME.test"
    TIMEFACTOR=1
  else
    if [ -f testset/$TSTNAME.ttest ]; then
      FULLTSTNAME="testset/$TSTNAME.ttest"
      TIMEFACTOR=$TIMELIMIT
    else
      if [ -f testset/$TSTNAME.test ]; then
...
COUNT=0
for INSTANCE in $(cat $FULLTSTNAME | awk '{print $1}'); do
  # if the key word DONE appears in the test file, skip the remaining test file
  if test "$INSTANCE" = "DONE"; then
    break
  fi
  # check if problem instance exists
  for IPATH in ${POSSIBLEPATHS[@]}; do
    #echo $IPATH
    if test "$IPATH" = "DONE"; then
      echo "input file $INSTANCE not found!"
    elif test -f $IPATH/$INSTANCE; then
...
COUNT=0
for TL in $(cat $FULLTSTNAME | awk '{print match($2, /[^ ]/) ? $2 : "'$TIMELIMIT'"}'); do
  TMPTL=$(($TL * $TIMEFACTOR))
  TIMELIMLIST[$COUNT]=$TMPTL
  # we add 100% to the hard time limit and additional 600 seconds in case of small time limits
  HARDTIMELIMIT=$(expr $(expr $TMPTL + 600) + $TMPTL)

  if test $TIMEFORMAT = "format"; then
