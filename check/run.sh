...
echo ">>> Executing: ${EXECNAME} ${PAPILO_OPT_COMMAND} -f ${FILENAME} -p ${SETTING_PATH} --tlim ${TIMELIMIT}"
eval "${EXECNAME} ${PAPILO_OPT_COMMAND} -p ${SETTING_PATH} -f ${FILENAME}" --tlim ${TIMELIMIT} 2>> "${ERRFILE}" | tee -a "${OUTFILE}"
echo "----------------------------------------------------------"
