...
echo ">>> Executing: ${EXECNAME} ${PAPILO_OPT_COMMAND} -f ${FILENAME} -p ${SETTING_PATH}"
eval "${EXECNAME} ${PAPILO_OPT_COMMAND} -p ${SETTING_PATH} -f ${FILENAME}" 2>> "${ERRFILE}" | tee -a "${OUTFILE}"
echo "----------------------------------------------------------"
