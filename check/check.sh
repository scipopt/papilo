...
if [[ $PAPILO_OPT_COMMAND != "solve" ]] && [[ $PAPILO_OPT_COMMAND != "presolve" ]]; then
...
if test -e $SCIPPATH/../$BINNAME; then
  ...
  for ((s = 0; $s <= $SEEDS; s++)); do
    ...
    for ((p = 0; $p <= $PERMUTE; p++)); do
      ...
      for SETNAME in "${SETTINGSLIST[@]}"; do
        ...
        if test "$SKIPINSTANCE" = "true"; then
          continue
        fi
